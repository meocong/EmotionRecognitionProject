...
def detectFaces(gray):
    ...
    faces2 = face_cascade2.detectMultiScale(gray, scaleFactor=1.1, minNeighbors=3, minSize=(5, 5),
                                          flags=cv2.CASCADE_SCALE_IMAGE)

    faces3 = face_cascade3.detectMultiScale(gray, scaleFactor=1.15, minNeighbors=3, minSize=(3, 3),
                                            flags=cv2.CASCADE_SCALE_IMAGE)
    res = []
...
def predict_emotion(face_image_gray): # a single cropped face
    ...
    return emotions_lists[list_of_list[0]]
    print(list_of_list)
